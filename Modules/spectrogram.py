...
def waterfallspect(data, fc, fs, fft_size, overlap_fac=0.0):
    ...
    hop_size = np.int32(np.floor(fft_size))
    ...
    for i in range(total_segments):
        ...
        segment = data[current_hop:current_hop +
                       fft_size]
        spectrum = fftshift(fft(segment)) / fft_size
        if (i == 0):
...
def SpectrogramPlot(SignalInfo, cmapstr):
    """
    ...
    for i in range(0, chunknumber):
        ...
        if(i == 0):
            waterfall_data_one_second = waterfallspect(
                signal_chunk_iq, fc, fs, nfft)
