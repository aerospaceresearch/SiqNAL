...
def SpectrogramPlot(SignalInfo,cmapstr,WaitWindow):
	...
	for i in range(0, chunknumber):
		...
		if (i == 0):
			spectrogram_data = ShortTimeSpectrogram(signal_chunk_iq,nfft, fs, fc)

		else:
			spectrogram_data = spectrogram_data + ShortTimeSpectrogram(signal_chunk_iq,nfft, fs, fc)
	...
	plt.imshow(display_data.T, extent=freq_vector + dummy_vector,origin='lower', interpolation='nearest', aspect='auto', cmap=cmap)
	plt.colorbar()
	plt.xlabel('Frequency (Hz)')
	plt.tight_layout()
	plt.yticks([])
